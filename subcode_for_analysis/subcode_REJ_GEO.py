...
def fetch_db(
    ...
    base_dir = "/INET/consent_discrepancy/work"
    ...
    # -----------------------------
    reject_machines = [
        "inet-c-banners-australia-0.dyn.mpi-klsb.mpg.de",
    ]
...
def get_stage_counts(df):
    ...
    df = df[['visit_id', "clicked", "domain", 'stage']]
    print(f"before size {df.shape[0]}")
    df = df[df["clicked"].notna()]
    print(f"after size {df.shape[0]}")
    df["clicked"] = df["clicked"].astype(str)
    df["clicked_stage"] = df["stage"].astype(str) + " (" + df["clicked"].astype(str).str.capitalize() + ")"
